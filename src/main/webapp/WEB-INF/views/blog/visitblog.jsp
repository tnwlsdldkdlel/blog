...
<script type="text/javascript">
	$(document).ready(
			function() {
				...
				//내 블로그로 이동
				$("#myblogmove").on("click",function(e){
				...
				<div class="w3-white">
					<div class="w3-container">
						...
						<div class="w3-container w3-center">
							<img alt="프로필이미지" src="/upload/${blogowner.profileImage }"
								class="rounded-circle" style="max-height: 200px">
						</div>
						<br />
						<p class="w3-center">
							<b><c:out value="${blogowner.nickname }" /></b> (
							<c:out value="${blogowner.id }" />
							)
						</p>
						<p class="w3-center">
							<c:out value="${blogowner.blog_itroduction }" />
						</p>
						<hr>
						<div class="w3-center">
							<div class="w3-center">
								<form action="/logoutCheck" method="post" id="logoutForm">
									<button type="submit" id="logout"
										...
										<button type="button" id="myblogmove"
										class="w3-button btn-secondary rounded-pill w3-center">내 블로그로 이동</button>
				...
				<c:forEach items="${list}" var="list">
					<div class="w3-white w3-margin " id="listDiv">
						...
						<div class="Blogintroduce w3-margin">
							<img alt="프로필이미지" src="/upload/${list.profileImage }"
								class="rounded-circle" style="max-height: 30px"/>
							<a href="/ITlog/blog/${list.id }"><c:out value="${list.id }" /></a>
							<div class="w3-right">
